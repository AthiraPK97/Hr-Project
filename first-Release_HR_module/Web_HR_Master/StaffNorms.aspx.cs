...
{
    public partial class StaffNorms : System.Web.UI.Page
    {
        ServiceReference1.WebService_HRMasterSoapClient objService = new ServiceReference1.WebService_HRMasterSoapClient();

        protected void Page_Load(object sender, EventArgs e)
        {
            DataTable dt_Normscount = objService.Get_Norms_Count().Tables[0];
            if (dt_Normscount.Rows.Count > 0)
            {
                GridView1.DataSource = dt_Normscount;
                GridView1.DataBind();
            }
            else
            {
        ...
        protected void ButtonExport_Click(object sender, EventArgs e)
        {
            ...
            Response.AddHeader("Content-Disposition", "attachment;filename = Staff_Norms_Report.xls");
